...
class contourPlotter:
	def __init__(self, plotName="test", xSize=800, ySize=600):
		...
		self.atlasLabel = "#bf{#it{ATLAS}} Internal"
	...
	def drawOneSigmaBand(self, band, color=ROOT.TColor.GetColor("#ffd700"), lineColor=ROOT.kBlack, alpha=0.35, legendOrder=0, title="Expected Limit"):
		self.canvas.cd()
		...
		tmpLegendObject.SetLineColor(lineColor)
		tmpLegendObject.SetLineStyle(7)
		tmpLegendObject.SetLineWidth(1)
		if type(legendOrder) == int:
			self.legendObjects.append( ( legendOrder, tmpLegendObject, title + " (#pm1 #sigma_{exp})", "lf" ) )
		return

	def drawExpected(self, curve, color=ROOT.kBlack, alpha=0.8, legendOrder=None, title="Expected Limit", drawOption="L"):
		self.canvas.cd()
	...
	def drawObserved(self, curve, title="Observed Limit (#pm1 #sigma_{theory}^{SUSY})", color=ROOT.TColor.GetColor("#800000"), alpha=0.8, legendOrder=1, drawOption="L"):
		self.canvas.cd()
		curve.SetLineColorAlpha(color,alpha)
	...
	def drawLine(self, coordinates, label = "", color = ROOT.TColor.GetColor("#b0b0b0"), style = 7, labelLocation=0 , angle=0):
		self.canvas.cd()
		tmpLine = ROOT.TLine()
	...
	def drawLabel(self, x = 0.23, y = 0.61, label = "test", color = ROOT.TColor.GetColor("#969393"), alpha = 0.6, font = 62, textSize=0.028 , angle=0):
		self.canvas.cd()
		latexObject = ROOT.TLatex()
		latexObject.SetTextSize(textSize)
		latexObject.SetTextFont(font)
		latexObject.SetTextColorAlpha(color,alpha)
		latexObject.DrawLatexNDC(x,y,label)
	...
	def decorateCanvas(self):
		...
		latexObject.SetTextSize(0.03)
		latexObject.SetTextFont(42)
		latexObject.DrawLatexNDC(0.23, 0.81, self.lumiLabel)

		latexObject.SetTextSize(0.04)
		latexObject.SetTextFont(42)
		latexObject.DrawLatexNDC(0.23, 0.86, self.atlasLabel)
		ROOT.gPad.RedrawAxis()
		ROOT.gPad.SetTicks()
		self.canvas.Update()
		return
