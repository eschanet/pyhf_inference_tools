...
class contourPlotter:
    ...
    def drawAxes(self, axisRange=[0, 0, 2000, 2000]):
        ...
        self.bottomObject.SetTitle(
            ";%s;%s" % (self.xAxisLabel, self.yAxisLabel)
        )
    ...
    def drawOneSigmaBand(
            self,
            band,
            color=ROOT.TColor.GetColor("#ffd700"),
            ...
            if type(legendOrder) == int:
                self.legendObjects.append(
                    (
                        legendOrder, tmpLegendObject,
                        title + " (#pm1 #sigma_{exp})", "lf"
                    )
                )
            return
...
>>>>>>> 10d8a9a1580fd7ad7c401b538ed3f059220c64ff
    ...
    def drawShadedRegion(
        ...
        if type(legendOrder) == int:
            self.legendObjects.append(
                (
                    legendOrder, curve.Clone("ShadedRegion_" + title), title,
                    "F"
                )
            )
        return
    ...
    def createLegend(self, shape=(0.22, 0.55, 0.65, 0.75)):
        ...
        for iItem, (legendOrder, item, title,
                    style) in enumerate(self.legendObjects):
    ...
    def drawTheoryLegendLines(
        ...
        tmpLine.SetLineStyle(style)
        tmpLine.DrawLineNDC(
            xyCoord[0], xyCoord[1], xyCoord[0] + length, xyCoord[1]
        )
        tmpLine.DrawLineNDC(
            xyCoord[0], xyCoord[1] + ySeparation, xyCoord[0] + length,
