...
parser.add_argument("--yVariable","-y",  type = str, default = "mn" )
...
parser.add_argument("--forbiddenFunction","-l", type=str, help="""a ROOT TF1 definition for a forbidden line e.g. kinematically forbidden regions. Set to 'None' to turn off.""", default=None)
...
parser.add_argument("--gridx","-g",      help = "use the grid-x points instead of the x12 grid points", action="store_true", default=False)
...
def main():
	...
	if not args.forbiddenFunction:
		...
		print (">>> ** use the option --forbiddenFunction none            **")
...
def harvestToDict( harvestInputFileName = "" , tmpListOfContours = listOfContours ):
	...
	if ".json" in harvestInputFileName:
		...
		with open(harvestInputFileName) as inputJSONFile:
			...
			def getX(mg,mc,mn):
			    return (float(mc)-float(mn))/(float(mg)-float(mn))
			...
			for sample in inputJSON:
				...
				try:
					if not args.gridx:
						...
						sampleParams = ((float(sample[args.xVariable]),getX(sample["mg"],sample["mc"],sample["mn"]) ))
				except:
...
def addValuesToDict(inputDict, function, numberOfPoints = 100, value = 0):
	...
	forbiddenFunctions = []
	if args.gridx:
		f1 = ROOT.TF1("forbiddenFunction${}".format(value),'1.01',lowerLimit,upperLimit)
		f1.Write("forbiddenFunction${}".format(value))
		forbiddenFunctions.append(f1)
		f0 = ROOT.TF1("forbiddenFunction${}".format(value),'-0.01',lowerLimit,upperLimit)
		f0.Write("forbiddenFunction${}".format(value))
		forbiddenFunctions.append(f0)
	else:
		...
		forbiddenFunctions.append(forbiddenFunction)
