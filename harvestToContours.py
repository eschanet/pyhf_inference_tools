...
parser.add_argument(
    ...
    type=str,
    help=
    "interpolation function for scipy (RBF): linear, cubic, gaussian, multiquadric or (griddata): nearest, linear, cubic",
    default="multiquadric"
)
parser.add_argument(
    "--interpolationScheme",
    type=str,
    help="type of interpolation for scipy: rbf, griddata",
    default="rbf"
)
parser.add_argument(
...
parser.add_argument(
    "--logX",
    help="use log10 of x variable",
    action="store_true",
    default=False
)
parser.add_argument(
    "--logY",
    help="use log10 of y variable",
    action="store_true",
    default=False
)
...
parser.add_argument(
    ...
    type=str,
    help=
    "give a json file with key=variable and value=value. e.g. use for pinning down third parameter in harvest list",
    default=""
)
parser.add_argument(
    "--forbiddenFunction",
    ...
    type=str,
    help=
    """a ROOT TF1 definition for a forbidden line e.g. kinematically forbidden regions. (defaults to diagonal, i.e. -l 'x'). Set to 'None' to turn off.""",
    default="x"
)
parser.add_argument(
    "--ignoreUncertainty",
...
parser.add_argument(
    ...
    type=str,
    help=
    "smoothing option. For ROOT, use {k5a, k5b, k3a}. For scipy, uses smoothing from RBF.",
    default="0"
)
parser.add_argument(
    "--noSig",
    "-n",
    help=
    "don't convert CLs to significance -- don't use this option unless you know what you're doing!",
    action="store_true",
    default=False
)
...
parser.add_argument(
    "--useUpperLimit",
    help=
    "use upper limit information instead of CLs. Automatically turns off significance transform.",
    action="store_true",
    default=False
)
...
parser.add_argument(
    ...
    "-b",
    help=
    "if contours are closed shapes in this space, this can help with stitching issues if you're seeing weird effects",
    action="store_true",
    default=False
)
...
try:
    ...
    print(
        '>>> >  lsetup "views LCG_94 x86_64-centos7-gcc62-opt" # or replace with a newer LCG release or more appropriate arch'
    )
    print(">>> ")
...
if args.useUpperLimit:
    ...
    print(">>> ")
    print(
        ">>> INFO: You've asked for interpolation of an upper limit plane. So I'll also turn off the significance transform (--noSig) for you. And setting level to 1."
    )
    print(">>> ")
    ...
    if args.ignoreUncertainty:
        ...
        listOfContours = [
            "upperLimit", "expectedUpperLimit", "expectedUpperLimitPlus1Sig",
            "expectedUpperLimitPlus2Sig", "expectedUpperLimitMinus1Sig",
            "expectedUpperLimitMinus2Sig"
        ]
    listOfContours_OneSigma = [
        "expectedUpperLimitPlus1Sig", "expectedUpperLimitMinus1Sig"
    ]
    listOfContours_TwoSigma = [
        "expectedUpperLimitPlus2Sig", "expectedUpperLimitMinus2Sig"
    ]
...
def main():
    ...
    if args.nominalLabel in args.inputFile:
        ...
        try:
            processInputFile(
                inputFile=args.inputFile.replace(args.nominalLabel, "Up"),
                outputFile=f,
                label="_Up"
            )
            processInputFile(
                inputFile=args.inputFile.replace(args.nominalLabel, "Down"),
                outputFile=f,
                label="_Down"
            )
        except:
    ...
    print(">>> ")
    print(
        ">>> All done! Have a beautiful day -- You're an inspiration you wonderful person you!"
    )
...
def processInputFile(inputFile, outputFile, label=""):
    ...
    if label == "_UL":
        ...
        upperLimitSurface = interpolateSurface(
            resultsDict,
            args.interpolation,
            args.useROOT,
            outputSurfaceTGraph=True,
            tmpListOfContours=["upperLimit"]
        )
    ...
    outputFile.mkdir("SubGraphs")
    outputFile.cd("SubGraphs")
    for whichContour in listOfContours:

        try:
            for iSubGraph, subGraph in enumerate(outputGraphs[whichContour]):
                subGraph.Write(
                    "%s_Contour_%d%s" % (whichContour, iSubGraph, label)
                )
        except:
            print(
                ">>> ... Well that one's no good. You might want to check on that... - %s"
                % whichContour
            )
    ...
    if not args.ignoreUncertainty and label == "":
        if len(outputGraphs[listOfContours_OneSigma[0]]
               ) == 0 and len(outputGraphs[listOfContours_OneSigma[1]]) > 0:
            print(">>>")
            ...
            print(">>> ")
            for icurve, curve1 in enumerate(
                outputGraphs[listOfContours_OneSigma[1]]
            ):
                band_1s = createBandFromContours(curve1)
                band_1s.SetFillColorAlpha(
                    ROOT.TColor.GetColor("#ffd700"), 0.75
                )
    ...
    if label == "":
        ...
        try:
            if not args.ignoreUncertainty and outputFile.Get("Band_1s_0"):
                for iGraph in xrange(
                    len(outputGraphs[listOfContours_OneSigma[1]])
                ):
                    outputFile.Get("Band_1s_%d" % iGraph
                                   ).Draw("ALF" if iGraph == 0 else "LF")
            else:
            ...
            print(
                ">>> WARNING: Had some problems making a canvas for you. Probably some curves are missing for some reason"
            )
...
def harvestToDict(harvestInputFileName="", tmpListOfContours=listOfContours):
    ...
    if ".json" in harvestInputFileName:
            ...
            for sample in inputJSON:
                ...
                if args.fixedParamsFile:
                    failConstraintCutList = [
                        ...
                        for (constrainedVar,
                             constrainedVal) in constraintsDict.iteritems()
                    ]
                ...
                for x in tmpListOfContours:
                    if not (
                        args.noSig
                        or x in ["upperLimit", "expectedUpperLimit"]
                    ):
                ...
                if not math.isinf(float(sample[expectedContour])):
                    tmpList = [
                        float(sample["%s" % x]) if (
                            args.noSig
                            or x in ["upperLimit", "expectedUpperLimit"]
                        ) else ROOT.RooStats.PValueToSignificance(
                            float(sample["%s" % x])
                        ) for x in tmpListOfContours
                    ]
                    modelDict[sampleParams] = dict(
                        zip(tmpListOfContours, tmpList)
                    )
                    if "fID" in sample:
                    ...
                    if not sampleParams in modelDict:
                        modelDict[sampleParams] = dict(
                            zip(
                                tmpListOfContours,
                                [args.sigmax for x in tmpListOfContours]
                            )
                        )
                        modelDict[sampleParams]["fID"] = ""
                if (args.debug):
                    print(
                        sampleParams, float(sample[observedContour]),
                        float(sample[expectedContour])
                        if args.noSig else ROOT.RooStats.PValueToSignificance(
        ...
        print(
            ">>> ... Interpreting text file -- (This feature hasn't been fully tested)"
        )
        print(">>> ... Also why aren't you just using JSON you fool")
        print(
            ">>> ... Seriously -- HF now spits out a JSON, and if you're converting that back to a text file you deserve to have things not work..."
        )
        ...
        try:
            ...
            print(
                ">>> Crash and burn -- I need a harvest tree description file!"
            )
            sys.exit(0)
        ...
        for model in harvestInput.readlines():
            ...
            sampleParams = (
                float(values[args.xVariable]), float(values[args.yVariable])
            )
            ...
            if args.fixedParamsFile:
                failConstraintCutList = [
                    not values[str(constrainedVar)] == constrainedVal
                    for (constrainedVar,
                         constrainedVal) in constraintsDict.iteritems()
                ]
                if any(failConstraintCutList):
            ...
            tmpList = [
                float(values["%s/F" % x]) if
                (args.noSig or x in ["upperLimit", "expectedUpperLimit"]) else
                ROOT.RooStats.PValueToSignificance(float(values["%s/F" % x]))
                for x in tmpListOfContours
            ]
...
def addValuesToDict(inputDict, function, numberOfPoints=100, value=0):
    ...
    forbiddenFunction = ROOT.TF1(
        "forbiddenFunction${}".format(value), args.forbiddenFunction,
    ...
    if value == "mirror":
        ...
        for xValue in [
            lowerLimit + x *
            (upperLimit - lowerLimit) / float(numberOfPoints * 100)
        ...
        for signalPoint in inputDict:
            closestPointOnLine = list(
                closest_point(
                    np.array([signalPoint]), np.array(forbiddenLineArray)
                )
            )
...
def interpolateSurface(
    ...
    tmpListOfContours=listOfContours
):
    ...
    for whichContour in tmpListOfContours:
        ...
        while any(
            [
                math.isinf(tmp) or math.isnan(tmp)
                for tmp in zValues[whichContour]
            ]
            ...
            if (args.debug):
                print(
                    ">>> ... Remove Inf or NaN at i=%d x=%d y=%d" % (
                        myindex, x[whichContour][myindex],
                        y[whichContour][myindex]
                    )
                )
    ...
    if useROOT:
        print(
            ">>> ... Using ROOT's internal interpolation scheme (triangular)."
        )
        print(
            ">>> ... If you like your plot to look like a dinosaur's back or an escalator, then you're entitled to that.."
        )

        canvas = ROOT.TCanvas("c1", "c1", 800, 600)
        h2D = ROOT.TH2D(
            "h", "h", 10000, min(x0), max(x0), 10000, min(y0), max(y0)
        )

        for whichContour in tmpListOfContours:
            graphs[whichContour] = createGraphsFromArrays(
                x[whichContour], y[whichContour], zValues[whichContour],
                whichContour
            )
        ...
        for whichContour in tmpListOfContours:
            ...
            if args.interpolationScheme.lower() == "rbf":
                yScaling = (np.max(xArray) - np.min(xArray)
                            ) / (np.max(yArray) -
                                 np.min(yArray)) if np.max(yArray) else 1
            ...
            try:
                if args.interpolationScheme.lower() == "rbf":
                    ...
                    ZI = scipy.interpolate.griddata(
                        (xArray, yArray),
                        zArray, (xymeshgrid[0], xymeshgrid[1]),
                        method=interpolationFunction
                    )

                else:
                    print(
                        ">>> ... ... scipy interpolation scheme not recognized. See -h for options."
                    )
                    sys.exit(1)
                ...
                print(
                    sorted(
                        zip(xArray, yArray, zArray), key=lambda x: x[0] * x[1]
                    )
                )
                sys.exit(1)
            ...
            if whichContour == expectedContour and outputSurface:
                print(
                    ">>> ... ... Writing out expected surface to pickle file"
                )
                with open(
                    args.outputFile + '.expectedSurface.pkl', 'w'
                ) as outfile:
                    pickle.dump(
                        {
                            "x": xymeshgrid[0],
                ...
                print(
                    ">>> ... ... Writing out observed surface to pickle file"
                )
                with open(
                    args.outputFile + '.observedSurface.pkl', 'w'
                ) as outfile:
                    pickle.dump(
                        {
                            "x": xymeshgrid[0],
            ...
            if outputSurfaceTGraph:
                return createTGraph2DFromMeshGrid(
                    xymeshgrid[0], xymeshgrid[1], ZI
                )

            # Turn this surface into contours!
            contourList = getContourPoints(
                xymeshgrid[0], xymeshgrid[1], ZI, args.level
            )

            graphs[whichContour] = []
            for contour in contourList:
                graph = ROOT.TGraph(
                    len(contour[0]), contour[0].flatten('C'),
                    contour[1].flatten('C')
                )
...
def createTGraph2DFromArrays(x, y, z):
    ...
    return ROOT.TGraph2D(len(x), array('f', x), array('f', y), array('f', z))
        ...
        if listOfFIDs != None:
            try:
                outputGraph.SetPoint(
                    imodel, model[0], model[1],
                    listOfFIDs.index(modelDict[model][myName])
                )
            except:
                print(
                    ">>> WARNING: Model point has a SR not in the list for some reason! Skipping, but check for problems in input JSON!"
                )
                continue
        else:
            value = modelDict[model][myName] if (
                args.noSig or myName in ["upperLimit", "expectedUpperLimit"]
            ) else ROOT.RooStats.SignificanceToPValue(
                modelDict[model][myName]
            )
...
def createBandFromContours(contour1, contour2=None):
    ...
    if not contour2:
        ...
        for iPoint in xrange(contour1.GetN()):
            ...
            outputGraph.SetPoint(
                contour2.GetN() + pointOffset + iPoint, tmpx, tmpy
            )
        ...
        if args.closedBands:
            ...
            if (
                ...
                pointOffset += 1
            outputGraph.SetPoint(
                contour1.GetN() + contour2.GetN() + pointOffset - 1, tmpx, tmpy
            )
